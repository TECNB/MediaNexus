...
export function QuarkIngestPanel() {
  ...
  async function handleSubmit() {
    ...
    setCreatedTask(null)
    try {
      const task = typeof selectedYear === 'number'
      ...
      <div className="min-w-0 space-y-6">
        ...
        shareUrl.trim() ? (
          <section className="space-y-3">
            <SectionHeading
              label="分享目录规划"
              title="选择保存范围并确认季度映射；改名预览和更新文件夹设置在两个入口中保持一致。"
            />
            <QuarkSeasonIngestWorkspace
              mediaType={mediaType as 'series' | 'variety'}
              shareUrl={shareUrl.trim()}
              title={selectedTitle}
              originalTitle={selectedItem.original_title}
        ...
        {!isSeasonMedia ? (
          ...
          >
            ...
            {submitStatus === 'loading' ? '正在创建并触发…' : '创建并立即执行 QAS 任务'}
          </Button>
        ) : null}
